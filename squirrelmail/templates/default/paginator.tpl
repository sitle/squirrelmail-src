...
function paginator($aTemplateVars) {
     static $bScriptAdded;
...
<?php
    if (isset($compact_paginator) && $compact_paginator) {
        $sPaginator = get_compact_paginator_str($mailbox, $iOffset, $iTotal, $iLimit, $bShowAll, $javascript_on, $page_selector);
    } else {
        $sPaginator = get_paginator_str($mailbox, $iOffset, $iTotal, $iLimit, $bShowAll, $page_selector, $page_selector_max);
    }
