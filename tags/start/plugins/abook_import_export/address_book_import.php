<?php
   ...
   **
   **   This was a part of the SSquirrelmail source tree and is being ported
   **   to a plugin.
   **/
...
/** commented out until a way to deal with file renaming is figured out.
...
**/
   ...
   // This loop sets up a table of the data they uploaded to save time while the user rearranges it.
	...
	do {
	   ...
	   if(count($data) >= 5) {	// This if ensures the minimum number of columns
	      $cols = count($data);	// so importing can function for all 5 fields
	   } else {
	      $cols = 5;
	   }		
	   $row++;
	   ...
	   if($flag == 0 && !$finish) {                            // Table header on initial import
	      echo "   <TH BGCOLOR=\"$color[9]\" HEIGHT=\"35\" COLSPAN=\"$colspan\" BGCOLOR=\"$color[11]\">";
