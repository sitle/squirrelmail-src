...
/* Object methods */
	...
	function method_edit() {
		...
		if (isset($this->foowd->user->objectid) && $this->updatorid == $this->foowd->user->objectid) { // author is same as last author and not anonymous, so can just update
		...
		if ($editForm->submitted()) {
			switch ($this->edit($editArea->value, !$newVersion->checked, $editCollision->value)) {
			case 1:
				$this->foowd->template->assign('success', TRUE);
				break;
			case 2:
				$this->foowd->template->assign('success', FALSE);
				$this->foowd->template->assign('warning', TRUE);
				break;
			default:
				$this->foowd->template->assign('success', FALSE);
				break;
