...
// check objectid, now in DB objects.
        while ($foowd->conn->DBSelect(NULL, array('objectid'), array('AND', 'objectid = '.$this->objectid, 'classid = '.$this->classid, 'workspaceid = '.$this->workspaceid), NULL, NULL, 1)) {
            if (!$allowDuplicateTitle) {
...
// try to update existing record
        if ($foowd->conn->DBUpdate($fieldArray, $conditionArray)) {
            $exitValue = 1;
        } else {
// if fail, write new record
            if ($foowd->conn->DBInsert($fieldArray)) {
                $exitValue = 2;
            } else {
// if fail, modify table to include indexes from class definition
                if ($query = $foowd->conn->DBSelect(NULL,
                                                    array('*'),
                                                    NULL,
                                                    NULL,
                                                    NULL,
                    ...
                    $record = $foowd->conn->getRecord($query);
                    $missingFields = array();
                    ...
                    if ($missingFields != NULL && $foowd->conn->DBAlterTable($missingFields)) {
                        if ($foowd->conn->DBUpdate($fieldArray, $conditionArray)) {
                            $exitValue = 3;
                        } elseif ($foowd->conn->DBInsert($fieldArray)) {
                            $exitValue = 4;
                        }
                    }
                }
...
/* delete object */
    ...
    function delete(&$foowd) { // remove all versions of an object from the database
        ...
        if ($foowd->conn->DBDelete($conditionArray)) {
            $foowd->track();
            return TRUE;
        } else {
            $foowd->track();
...
/* view object */
    ...
    function method_view(&$foowd) {
        ...
        $foowd->tpl->assign_by_ref('STATUS_OK',
                                   _("Viewing Object Contents"));

        $foowd->tpl->assign('BODY', show($this));
...
/* object history */
    ...
    function method_history(&$foowd) {
        ...
        $history = new smdoc_display('object.history.tpl');

        $history->assign('created', date(DATETIME_FORMAT, $this->created));
        $history->assign('creator', $this->creatorName);
        $history->assign('type',    getClassDescription($this->classid));
        $history->assign('language',$this->workspaceid);
