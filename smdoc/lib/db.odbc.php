...
function databaseOpen($host, $user, $pass, $name) {
	if ($conn = odbc_connect($host, $user, $pass)) {
		return $conn;
	} else {
...
function getFieldType($query, $index) {
	return odbc_field_type($query, $index);
}
...
/*		if ($orders) {
		...
		$query = odbc_exec($conn, $SQLString) or error('Database query failed');		
	} else {
		$query = false;
	}
	if ($query && odbc_num_rows($conn) > 0) {
		return $query;
	} else {
		return false;
	}
...
function DBInsert(&$conn, $table, $fields) {
	...
	if ($table && $fields) {
		...
		$SQLString .= ')';
		if (DEBUG) DBTrack($SQLString);
		if ($query = odbc_query($SQLString, $conn)) {
			return true;
		} else {
			return false;
		}
...
function DBUpdate(&$conn, $table, $fields, $conditions) {
	...
	if ($table && $fields) {
		...
		if (DEBUG) DBTrack($SQLString);
		if ($query = odbc_query($SQLString, $conn) && getAffectedRows() > 0) {
			return true;
		} else {
			return false;
		}
...
function DBDelete(&$conn, $table, $conditions) {
	...
	if ($table) {
		...
		if (DEBUG) DBTrack($SQLString);
		if (odbc_query($SQLString, $conn) or error('Database query failed')) {
			return true;
		} else {
			return false;
		}
